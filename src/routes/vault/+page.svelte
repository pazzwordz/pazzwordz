...
<UnlockVaultModal bind:this={unlockVaultModal} supabase={supabase} userId={userId}/>
<div>
    <div>
    {#if $vaultKeyStore != undefined}
        Unlocked
        {:else}
        <button on:click={unlockVaultModal.show}>Unlock</button>
    {/if}
    </div>
    <div>
        {#if $vaultKeyStore != undefined}
            ...
            {#each entries as entry}
                <tr>
                    ...
                    <td>
                        ...
                        {:else }
                            <div>
                                <button disabled={$vaultKeyStore == undefined} on:click={() => decrypt(entry)}>Decrypt</button>
                            </div>
                        {/if}
                    </td>
                </tr>
